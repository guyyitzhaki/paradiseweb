<?php include 'header.php';?>
<div class="container">

       <h1 class="title">About Paradise Inn</h1>
       <div class="row">
              <div class="col-sm-12">
                     <p style="font-style: italic;">
                            "Paradise Inn, Talbiyeh, Jerusalem. A dream for only one person. Infinite stars and a real gate to paradise. Reserve your room at the former Lepers home. Jerusalem, where any piece of land can become your temporary dwelling."
                     </p>
                     <p>
                     In the frame of the development of the tourist policy of the city of Jerusalem, a group of artists based on the former Jerusalem Leper's home decided to be part of this new trend and generate a new resource for tourism and independent incomes for local artists and the Mamuta Art and Media Center at Hansen House: The Paradise Inn.
                     </p>
                     <p>
                            With a "green" approach to urban design and under the ideas of recycling electronic waste beside the former leper's gardens, the artists decided to create a summer hotel for temporary dwelling, a project that may synthesize the new visionaries ideas for a modern Jerusalem: a hotel for only one person, a personal paradise, built on the paradisiac gardens of the former lepers’ home, today rhthe Jerusalem Center for Art, Design and Technology.<br>
                            In this democratic project the artist aim to give to everyone the possibility to spend some nights in the historical gardens of the former "Jesus Hilfe" leprosarium.
                     </p>
                     <p>
                            Paradise Inn is an island, surrounded by walls, inside the walls of the former lepers home protecting historically the society from lepers and the lepers from the society. A metaphore of differentiation and exclusivity luxurious paradise expropiated from a public institution this time.
                            The Paradise Inn develops in the spirit of gentrification, invited one artists to reside for free in the paradise for a month an a half, expecting from him to produce many artworks for the comfort of the future customers.
                     </p>
              </div>
       </div>
       <!--<div class="row">-->

       <!--<div class="col-sm-4"><p>Fale acid rest bock, " trappist biere de garde," infusion oxidized, carboy. hard cider, crystal malt hoppy pitch alpha acid dry stout tulip glass fermentation. bitter malt extract, dunkle, squares. units of bitterness trappist wort chiller keg krausen ester. brewpub. cold filter length, alcohol copper degrees plato aau.</p></div>-->
              <!--<div class="col-sm-4"><p>Dry stout aau pilsner. bock cold filter lauter tun specific gravity degrees plato ibu cold filter. brew kettle crystal malt, hard cider ipa bright beer caramel malt. pitching aerobic keg finishing hops pub units of bitterness.</p></div>              -->
       <!--</div>-->
       <!--<div><p></p></div>-->
       <div class="row">
              <div class="col-sm-12" style="font-size: smaller;">
                     <p>&nbsp;</p><p>&nbsp;</p>
              <div style="padding-bottom: 20px;">This project was made possible thanks to</div>
              <ul style="list-style-type: none;">
                     <li class="links"><a href="http://www.jda.gov.il/" target="_blank"><img src="images/logos/jda2.png" style="max-width:100px;"><span style="padding-left: 10px;">Jerusalem Development Authority</span></a></li>
                     <li class="links"><a href="http://www.hansen.co.il/" target="_blank"><span style="width:100px"><img src="images/logos/hansen2.png" style="max-width:100px;max-height: 55px;"></span><span style="padding-left: 10px;">Hansen House</span></a></li>
                     <li class="links"><a href="http://mcs.gov.il/Culture/Pages/default.aspx/" target="_blank"><img src="images/logos/tarbut2.png" style="max-width:100px;max-height: 55px;"><span style="padding-left: 10px;">Ministry of Culture and Sport - Culture Authority</span></a></li>
                     <li class="links"><a href="http://www.mamuta.org/" target="_blank"><img src="images/logos/mamuta2.png" style="max-width:100px;"><span style="padding-left: 10px;">Mamuta art and media center</span></a></li>
              </ul>
              </div>
       </div>

       <div class="spacer">
       <div class="embed-responsive embed-responsive-16by9"><iframe  class="embed-responsive-item" src="//player.vimeo.com/video/55057393?title=0" width="100%" height="400" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe></div>
       </div>




</div>
<?php include 'footer.php';?>